...
class ShapeConfig:
    ...
    def status_payload(self) -> dict[str, Any]:
        return {
            ...
            "coil_locations_mm": [
                value * 1000.0 for value in self.coil_locations_m
            ],
...
def parse_shape_config(data: str) -> ShapeConfig:
    ...
    if not isinstance(locations_raw, list) or len(locations_raw) < 2:
        raise ShapeConfigError(
            "coil_locations_mm must contain at least two values"
        )
    locations_mm = sorted(
    ...
    if locations_mm[0] < 0.0 or locations_mm[-1] > length_mm:
        raise ShapeConfigError(
            "coil locations must lie within the catheter length"
        )
    ...
    nodes = tuple(
        min(
            n_sections,
            max(0, int(math.floor(value / section_length_mm + 0.5))),
        )
        for value in locations_mm
    )
    if len(set(nodes)) != len(nodes):
        raise ShapeConfigError(
            "multiple coils map to the same estimator node; increase "
            "n_sections "
